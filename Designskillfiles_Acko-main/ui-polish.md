...
Never remove focus indicators. Use the canonical `--shadowFocusRing` token (defined in `shadows.md` as `0 0 0 3px var(--colorPrimaryRing)`) — not `outline`. This keeps focus consistent with the rest of the shadow system and avoids subpixel rendering issues at the element corner.

```css
:focus-visible {
  box-shadow: var(--shadowFocusRing);
...
| Stop | Token | Role |
|------|--------|------|
| Top (0%) | `--colorPrimarySubtle` | Light purple tint |
| Bottom (100%) | `--colorSurface` | Page background — seamless blend |
...
<section
  style={{
    background:
      "linear-gradient(180deg, var(--colorPrimarySubtle) 0%, var(--colorSurface) 100%)",
  }}
>
...
See `colors-semantic.md` for the full dark theme token mappings.
